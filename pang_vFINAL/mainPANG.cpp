...
//estado del juego
enum GameState { START_SCREEN, PLAYING, GAME_OVER, WIN_SCREEN };
...
ScorePopUp* scorePopup = NULL; // puntero al popup de puntuación
...
GLuint winTexture;  // Texture data structure
...
int main(int argc,char* argv[])
{
  ...
  winTexture = LoadTexture(800, 600, "PANG_win_background_resized.bmp");
...
void printdata()
{
  ...
  snprintf(buffer, sizeof(buffer), "                 Lives: %d       Score: %d", nMan, score);
  l=strlen(buffer); 
...
{ 
  ...
  case WIN_SCREEN:
      // Dibuja la imagen del fondo
      drawBackgroundFullscreen(winTexture, WINX, WINY);
      worldobjects.draw();   // dibuja estático
      printdata();
      //game over text
      for (auto c : std::string("       PRESS R TO RESTART"))
...
{ 
  ...
  switch (key)
  {
    ...
    case 'r':  // Reinicia en Game Over
        if (gameState == GAME_OVER || gameState == WIN_SCREEN) {
