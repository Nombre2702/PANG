...
void Shape::move()
{
  ...
  // updates speed
  tspeed[Y] += 0.0007*GRAV*acc_ratio;
  
  // check world boundaries
  if (pos[X] <= -SIZEX2) { pos[X] = -SIZEX2; tspeed[X] = -0.1 * tspeed[X]; }
  if (pos[Y] <= -SIZEY2) { pos[Y] = -SIZEY2; tspeed[Y] = 0.3 * BALL_SPEED * (RAND_FRAC() + 0.2); }// -tspeed[Y];}
  //if(pos[Z]<=-SIZEZ2) {pos[Z] = SIZEZ2; tspeed[Z] = -tspeed[Z];}
  
  if(pos[X]>=SIZEX2) {pos[X] = SIZEX2; tspeed[X] = -tspeed[X];}
  if(pos[Y]>=SIZEY2) {pos[Y] = SIZEY2; tspeed[Y] = -0.1*BALL_SPEED;}
