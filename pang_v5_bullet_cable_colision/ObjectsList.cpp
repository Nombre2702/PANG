...
float ObjectsList::collisions(Bullet* theBullet, Man* theMan) {
	...
	while (current != NULL) {
		...
		if (ball) {
			...
			if (theBullet) {
				...
				if (distance_tip < hit_tip || distance_cable < hit_cable) {
					//if collision detected with the bullet top (cone)
		...
		if (animalito && theBullet) {
			...
			animalito->getPos(animalitoPos);	//get the position of the animalito
			theBullet->getPos(bulletPos);	//get the position of the bullet
			...
			float distance_tip_animalito = *animalito + theBullet; //get the distance between the animalito and the bullet
			float hit_tip_animalito = animalito->getSize() + theBullet->getSize();

			float hit_radius_animalito = animalito->getSize() + theBullet->getCableRadius();
			...
			if (distance_tip_animalito < hit_tip_animalito || distance_cable_animalito < hit_cable_animalito) {
				//collision detected with the bullet top (cone)
